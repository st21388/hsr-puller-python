...
# reading file compatibility with VS code (previously used library 'json')
script_dir = os.path.dirname(os.path.abspath(__file__))
history_file_path = os.path.join(script_dir, "history.json")
...
class MainWindow(Tk): # parameter Tk is used to create the window in super()
    ...
    def banner_one(self): # display banner 1
        with open(history_file_path) as f: # update the json (because history-window might have changed something)
            self.data = json.load(f)
    ...
    def pull(self, option):
        ...
        if self.current_banner == 0: # if a banner has not been selected
            ...
            if int(self.varLabel_currency.cget('text')) >= 160: # if able to afford 160 gems
                ...
                with open(history_file_path) as f: # update data with info from the json in case history-window has changed anything
                    self.data = json.load(f)
            ...
            if int(self.varLabel_currency.cget('text')) >= 1600: # if able to afford 1600 gems
                ...
                for i in range(0,10,1): # run rate calculation 10 times
                    self.pull_append()
                with open(history_file_path, 'w') as f: # append to json
                    json.dump(self.data, f, indent=1)
    ...
    def on_history_window_close(self): # when HistoryWindow is closed
        ...
        with open(history_file_path) as f: # if the clear history button has been pressed, re-update the json
            self.data = json.load(f)
...
class HistoryWindow(Toplevel): # toplevel is used here instead of tk as that is the tkinter module for opening an extra window
    ...
    def clear_history(self): # button for clearing json history
        ...
        with open(history_file_path, 'w') as f: # rewrite the json to be cleared
            json.dump(self.data, f, indent=1)
