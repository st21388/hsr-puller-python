...
Component 1: Version 3:
Edit the json file to store history for 3 different banners. The calculator
should now instead ask for which banner the user is on, then add information to
the history for that specific banner (banner 1, 2 or 3)
"""
import json
import random
...
def pull(curr_banner): # function for pulling
    data[curr_banner]["warp_4"] += 1 # increment 4 & 5 star pity count by 1
    data[curr_banner]["warp_5"] += 1
    rate_4 = get_rate_4_star(data[curr_banner]["warp_4"]) # calculate the rates
    rate_5 = get_rate_5_star(data[curr_banner]["warp_5"])
    ...
    if outcome == "5&4": # if both 5 and 4 star win, discard 4 star
        ...
        data[curr_banner]["history"].append([5,data[curr_banner]["warp_5"],data[curr_banner]["warp_4"]])
        data[curr_banner]["warp_5"] = 0
        data[curr_banner]["warp_4"] = 0
        print("5 STAR WIN!")
        ...
        data[curr_banner]["history"].append([4,data[curr_banner]["warp_5"],data[curr_banner]["warp_4"]])
        data[curr_banner]["warp_4"] = 0
        print("4 STAR WIN!")
    else: # else, then 3 star was won
        data[curr_banner]["history"].append([3,data[curr_banner]["warp_5"],data[curr_banner]["warp_4"]])
        print("3 STAR")
    
    if len(data[curr_banner]["history"]) == 300:
        data[curr_banner]["history"].pop(0) # history can not store over 300 pulls    
...
while True:
    banner = input("Enter current banner: ")
    if banner == "clear": # debug for clearing the history.json dictionary
        data = {
            "1": {
                "history": [],
                "warp_4": 0,
    ...
    elif int(banner) > 0 and int(banner) <= 3:
        data["banner"] = banner # change the current banner in the json
        ...
        print("Enter a valid banner (1, 2, or 3)")
